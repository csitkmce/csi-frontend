...
<div class="flex min-h-screen w-full max-w-7xl items-center border-x-1 border-black bg-[#222222] max-md:flex-col">
  <div class="flex w-full flex-col items-center p-4">
    {#if posterUrl}
      <img
        src={posterUrl}
        alt="Event poster"
        class="border max-sm:w-80 min-sm:max-w-[400px] mb-4"
      />
    {/if}

    <h2 class="text-2xl font-bold mb-2">{event.name}</h2>
    <p class="mt-2 text-gray-400 text-lg min-sm:text-justify">
      {event.description}
    </p>
    ...
    <div class="mt-4 space-y-1 text-white">
      ...
      <p>Registration: {new Date(event.regStart).toLocaleString()} – {new Date(event.regEnd).toLocaleString()}</p>
